...
exports.createPoll = async(req,res)=>{
    ...
    try{
        ...
        switch(type){
            case "single-choice":
                if(!options || options.length <2){
                    return res.status(400).json({message:"Single-choice poll must have atleast two options"});
                }
            ...
            case "image-based":
                if(!options || options.length <2){
                    return res.status(400).json({
                        message:"Image-based poll must have atleast two image URLs",
                    });
                }
      ...
      res.status(500).json({messaeg:"Error registring user",error:err.message})
    }
}
...
exports.getAllPolls=async(req,res)=>{
  ...
  try{
    ...
    .populate({                                                 //    .populate("responses.voterId","username profileImageUrl fullName") Works same but preferred is the other one
    ...
    const updatedPolls=polls.map((poll)=>{
        ...
        return {
            ...poll.toObject(),  //AS it is a mongodb doc so to convert it into appropriate form
            userHasVoted,
        }
    });
    ...
    //ensure all types are included in stats
    const allTypes=[
    ...
    res.status(500).json({messaeg:"Error getting all Polls",error:err.message});
  }
}
...
exports.getVotedPolls=async(req,res)=>{
    ...
    try{
        ...
        const polls=await Poll.find({voters:userId}) //filter by polls where the user's id exists in the voters
      ...
      res.status(500).json({messaeg:"Error getting voted polls",error:err.message});
    }
}
...
exports.getPollById=async(req,res)=>{
    ...
    try{
      ...
      res.status(500).json({messaeg:"Error getting Poll by ID",error:err.message});
    }
}
...
exports.closePoll=async(req,res)=>{
    ...
    try{
        const poll=await Poll.findById(id);
        if(!poll){
            return res.status(400).json({message:"Poll not ofund"});
        }

        if(poll.creator.toString()!== userId){
            return res.status(403).json({message:"Youa re not authorized to close this poll"});
        }
      ...
      res.status(500).json({messaeg:"Error Closing poll",error:err.message});
    }
}
...
exports.bookmarkPoll=async(req,res)=>{
    ...
    try{
        ...
        //check if poll is laready bookmarked
        ...
        if(isBookmarked){
            ...
            await user.save();
            return res.status(200).json({message:"Poll removed from bookmarks",bookmarkedPolls:user.bookmarkedPolls})
        }
      ...
      res.status(500).json({messaeg:"Error bookmarking the poll",error:err.message});
    }
}

//Get all Bookmarked Poll
exports.getBookmarkedPolls=async(req,res)=>{
    ...
    try{
      ...
      res.status(500).json({messaeg:"Error getting bookmarked polls",error:err.message});
    }
}
...
exports.deletePoll=async(req,res)=>{
    ...
    try{
      ...
      res.status(500).json({messaeg:"Error deleting poll",error:err.message});
    }
}
